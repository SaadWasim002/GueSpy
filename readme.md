...
- **UI**:
    ...
    - **Future Considerations**:
        - Support for random category selection.
...
- **API Endpoint (Select Category)**: `POST http://localhost:8080/category/select`
    - **Request Headers**: `X-User-Id: <userId>`, `Authorization: Bearer <token>` (Frontend will extract `userId` from JWT)
...
- **API Endpoint (Get All Groups)**: `GET http://localhost:8080/group/get`
    - **Request Headers**: `Authorization: Bearer <token>`
    - **Success (200 OK)**: Display the list of groups.
...
- **API Endpoint**: `GET http://localhost:8080/config/get`
    - **Request Headers**: `Authorization: Bearer <token>`
    - **Expected Response**: A list of configurations, including `key: "max_player_allowed_in_group"`, `key: "min_player_allowed_in_group"` (assuming this will be added), and `key: "max_group_allowed"`.
    - **Logic**:
...
- **UI**:
    ...
    - **Screen Type: `PASS_DEVICE`**:
        ...
        - Visually represent a player (e.g., an avatar or icon).
    ...
    - **Screen Type: `ROLE_REVEAL`**:
        ...
        - **For Non-Spies (`screenData.playerDetails.isSpy: false`)**:
            ...
            - Displays the `screenData.roleDescriptionText`.
...
- A modern, component-based JavaScript framework React is recommended for building the UI.
...
- Implement request interceptors to automatically attach the `Authorization` headers.
- Implement response interceptors to handle global error conditions (e.g., 401 unauthorized, 500 internal server error).
